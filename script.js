...
const aTab = document.getElementById("a-tab-btn");
const mTab = document.getElementById("m-tab-btn");
const aGenreBox = document.getElementById("a-genre-box");
const mGenreBox = document.getElementById("m-genre-box");
const aGenreBtn = document.getElementById("a-genre-btn");
const mGenreBtn = document.getElementById("m-genre-btn");
const aGenreList = document.getElementById("a-genre-list");
const mGenreList = document.getElementById("m-genre-list");
const aInfo = document.getElementById("a-info");
const mInfo = document.getElementById("m-info");
const aRatedChk = document.getElementById("a-rated-chk");
//I dont hate long variabel letsss gooo \0-0/. Do you ?
const mRatedChk = document.getElementById("m-rated-chk");
const searchBtn = document.getElementById("search-btn");
const posterImg = document.getElementById("poster-img");
const posterPh = document.getElementById("poster-ph");
...
const unameDisp = document.getElementById("uname-disp");
...
const loginPwEye = document.getElementById("login-pw-eye");
const signupPwEye = document.getElementById("signup-pw-eye");
const savedAnimeTab = document.getElementById("saved-anime-tab");
const savedMangaTab = document.getElementById("saved-manga-tab");
var savedActiveType = "anime";
...
(function () {
    ...
    function paint() {
        imgBox.style.transform =
            "translateX(0) scale(" + currentScale.toFixed(3) + ")";
    }
    imgBox.addEventListener("mouseenter", function () {
        ...
        imgBox.classList.remove("returning");
        imgBox.classList.add("hovering");
    ...
    imgBox.addEventListener("mouseleave", function () {
        currentScale = 1;
        imgBox.classList.remove("hovering");
        imgBox.classList.add("returning");
        paint();
        clearTimeout(settleTimer);
    ...
    //i like stars
    var starLayer = document.getElementById("star-layer");
    if (!starLayer) {
...
aGenreBtn.addEventListener("click", function () {
    aGenreList.classList.toggle("popup-is-open");
})
//doing the same for manga as welllll
mGenreBtn.addEventListener("click", function () {
    mGenreList.classList.toggle("popup-is-open")
})
//when someone clicks the anime tab, show anime stuff and hide the manga stuff
aTab.addEventListener("click", function () {
    aTab.classList.add("active-tab");
    mTab.classList.remove("active-tab");
    aGenreBox.style.display = "flex";
    mGenreBox.style.display = "none";
    aInfo.style.display = "block";
    mInfo.style.display = "none";
    aGenreList.classList.remove("popup-is-open");
    mGenreList.classList.remove("popup-is-open");
    resetCard();
});
mTab.addEventListener("click", function () {
    mTab.classList.add("active-tab");
    aTab.classList.remove("active-tab");
    mGenreBox.style.display = "flex";
    aGenreBox.style.display = "none";
...
function getPickedGenres() {
    let whichPopup;
    if (aTab.classList.contains("active-tab")) {
        whichPopup = aGenreList;
    } else {
        whichPopup = mGenreList;
    }
...
document.addEventListener("click", function (event) {
    const clickedInsideAnime = aGenreBtn.contains(event.target) || aGenreList.contains(event.target);
    const clickedInsideManga = mGenreBtn.contains(event.target) || mGenreList.contains(event.target);
...
document.addEventListener("click", function (event) {
    const clickedInsideMenu = menuBtn.contains(event.target) || menuDrop.contains(event.target);
    if (!clickedInsideMenu) {
        menuDrop.classList.remove("show");
    }
})
mylistBtn.addEventListener("click", function () { menuDrop.classList.remove("show"); });
logoutBtn.addEventListener("click", function () { menuDrop.classList.remove("show"); });
//if you really want the high rated .. then here you go
function bestRatedChecked() {
    if (aTab.classList.contains("active-tab")) {
        return aRatedChk.checked;
    } else {
        return mRatedChk.checked;
    }
}
var jikanGenreId = {
    "Action": 1,
...
closeTrailer.addEventListener("click", hideTrailer);
...
function resizeCard(ratio) {
    ...
    var maxHeight = sBody.clientHeight;
    if (!maxHeight) {
        maxHeight = window.innerHeight * 0.6;
    }
    var maxWidth = sBody.clientWidth - gap - infoMinWidth;
    if (maxWidth < 200) {
        maxWidth = 220;
    }
...
function showPoster(data, kind) {
    ...
    if (data && data.poster) {
        ...
        posterImg.onerror = function () {
            ...
            if (kind === "anime") {
                posterPh.innerHTML = ANIME_LOGO_SVG;
            } else {
                posterPh.innerHTML = MANGA_LOGO_SVG;
            }
...
function displayPick(data, kind) {
    ...
    if (data.trailer) {
        activeBox.querySelector("#watchTrailerBtn").addEventListener("click", function () {
            trailerFr.src = data.trailer + "?autoplay=1";
...
function currentKind() {
    if (mTab.classList.contains("active-tab")) {
        return "manga";
    }
    return "anime";
}
...
function syncNavButtons() {
    ...
    fwdBtn.disabled = hist.index >= hist.list.length - 1;
}
...
async function pull() {
    ...
    searchBtn.innerText = "Recommend Me";
}
searchBtn.addEventListener("click", pull);
...
const verifyFormWrap = document.getElementById("verify-wrap");
const verifyCodeInput = document.getElementById("verify-code");
const verifySubmitBtn = document.getElementById("verify-submit");
const verifyError = document.getElementById("verify-err");
...
function swapAuthForm(which) {
    ...
    var formGoingAway = signupWrap;
    var formComingIn = loginWrap;
    if (which === "signup") {
        formGoingAway = loginWrap;
        formComingIn = signupWrap;
    }
...
function paintAuthUI() {
    ...
    if (currentUser) {
        ...
        unameDisp.style.display = "inline";
        if (currentUser.verified) {
            unameDisp.textContent = "Hi, " + currentUser.username;
        } else {
            unameDisp.textContent = "";
            unameDisp.appendChild(document.createTextNode("Hi, " + currentUser.username + " "));
            var badge = document.createElement("a");
            badge.href = "#";
            badge.id = "unverif-badge";
            badge.textContent = "(unverified, click to verify)";
            unameDisp.appendChild(badge);
...
loginSubmit.addEventListener("click", function () {
    loginErr.textContent = "";
    var identifier = loginId.value.trim();
    var password = loginPw.value;
    if (!identifier || !password) {
        loginErr.textContent = "c'mon fill in both fields.";
        return;
    }
    ...
    fetch("Backend/login.php", {
        ...
        .then(function (json) {
            if (json.success) {
                ...
                loginErr.textContent = json.error || "login failed.";
            }
        })
        .catch(function () {
...
signupUname.addEventListener("keydown", function (e) {
    if (e.key === "Enter") {
        signupSubmit.click();
    }
});
...
signupSubmit.addEventListener("click", function () {
    signupErr.textContent = "";
    var username = signupUname.value.trim();
    var email = signupEmail.value.trim();
    var password = signupPw.value;
    if (!username || !email || !password) {
        signupErr.textContent = "c'mon fill in all fields.";
        return;
    }
    ...
    fetch("Backend/register.php", {
        ...
        .then(function (json) {
            if (json.success) {
                ...
                signupErr.textContent = json.error || "sign up failed.";
            }
        })
        .catch(function () {
            signupErr.textContent = "something went wrong. try again.";
        })
        .finally(function () {
...
function loadSavedList() {
    savedGrid.innerHTML = "";
    savedEmpty.textContent = "Dude, You haven't saved anything yet.";
    savedEmpty.style.display = "none";
    if (!currentUser || !currentUser.verified) {
        var localItems = loadLocalSaves().filter(function (it) {
            return it.item_type === savedActiveType;
        });
        ...
        localItems.forEach(function (item) {
            savedGrid.appendChild(buildSavedCard(item));
        });
        return;
    }
    fetch("Backend/get_saved_items.php?type=" + savedActiveType)
        .then(function (res) { return res.json(); })
        .then(function (json) {
        ...
        .catch(function () {
            savedEmpty.textContent = "Couldn't load your list buddy. Try again later.";
            savedEmpty.style.display = "block";
        });
}
...
savedAnimeTab.addEventListener("click", function () {
    savedActiveType = "anime";
    savedAnimeTab.classList.add("active-tab");
    savedMangaTab.classList.remove("active-tab");
    loadSavedList();
});
savedMangaTab.addEventListener("click", function () {
    savedActiveType = "manga";
    savedMangaTab.classList.add("active-tab");
    savedAnimeTab.classList.remove("active-tab");
    loadSavedList();
})
...
setupPasswordToggle(loginPwEye, loginPw);
setupPasswordToggle(signupPwEye, signupPw);
checkSession();